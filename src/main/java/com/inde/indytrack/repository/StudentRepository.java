...
public interface StudentRepository extends JpaRepository<Student, Long> {
    @Query(
        ...
        nativeQuery = true
    ) public List<Student> findByFirstOrLastName(@Param("name") String name);
    ...
    @Query(
        value = "SELECT m.name as minorName, SUM(c.credit_value) as creditsEarned, ROUND(SUM(c.credit_value) / 3.0 * 100, 2) as percentageCompleted " + 
                ...
                nativeQuery = true
    ) List<MinorProgressDTO> findIntendedMinorsProgress(@Param("studentId") Long studentId, @Param("minorName") String minorName);
}
